...
class Model  
{
    ...
    public function savefile($model, $inputField, $subfolder){
        $target_dir = "uploads/".$subfolder."/";
        // $new_name = $this->id.'_'.md5(time()).'.'.explode('.',basename($_FILES["$inputField"]["name"]))[1];die($new_name);
        $target_file = $target_dir . basename($_FILES["$inputField"]["name"]);
        $uploadOk = 1;
        $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
        // Check if image file is a actual image or fake image
        if(isset($_POST["submit"])) {
            $check = getimagesize($_FILES["$inputField"]["tmp_name"]);
            if($check !== false) {
                $uploadOk = 1;
            } else {
        ...
        // Check file size
        if ($_FILES["$inputField"]["size"] > 500000) {
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["$inputField"]["tmp_name"], $target_file)) {
